...
async function searchCommand(query) {
  ...
  if (!query) {
    ...
    console.log(`  ${dim('Usage:')} awesome-ui search ${cyan('<query>')}`);
    console.log(`  ${dim('Example:')} awesome-ui search gradient`);
    console.log('');
    return;
  }
  ...
  if (results.length === 0) {
    ...
    console.log(`  ${dim('Try a broader term or run')} awesome-ui list ${dim('to browse all.')}`);
    console.log('');
    return;
  }
