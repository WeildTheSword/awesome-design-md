...
async function fetchCommand(key) {
  ...
  if (!key) {
    ...
    console.log(`  ${dim('Usage:')} awesome-ui fetch ${cyan('<key>')}`);
    console.log(`  ${dim('Example:')} awesome-ui fetch gradient-wave`);
    console.log('');
    console.log(`  ${dim('Run')} awesome-ui list ${dim('to see all available components.')}`);
    console.log('');
    process.exit(1);
  }
